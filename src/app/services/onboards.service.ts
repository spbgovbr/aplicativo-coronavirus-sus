...
export class OnboardsService {
  ...
  getCDN(objectId): Promise<any> {
    return new Promise<any>((resolve) => {
      ...
      this.http.get(encodeURI(URL_CDN + '/onboad?where=' + params),
        {headers: apiHeader, withCredentials: false}).toPromise().then((response: any) => {
        resolve(response.results);
      }).catch(() => {
        this.http.get('assets/data/onboards-' + this.storage.get('LANGUAGE') + '.json').toPromise().then((response: any) => {
